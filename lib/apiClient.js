...
// Tombol cepat "Pakai Stok" di halaman Inventaris — kurangi stok item
// sebanyak `qty` tanpa perlu masuk mode edit penuh.
export async function apiUseInventoryStock(id, qty) {
  const res = await fetch(`/api/inventory/${id}/use`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ qty }),
  });
  if (!res.ok) throw new Error(await extractError(res, "Gagal mengurangi stok"));
  return res.json();
}
